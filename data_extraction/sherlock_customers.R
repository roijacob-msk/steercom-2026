...
{
  sherlock_run_august <- read_sheet("https://docs.google.com/spreadsheets/d/1qwTYn2Pvh4txtF1x0SkyOB1cweXtykDTrLY4Vp7i7qY/edit", sheet = "Borrower Details") |> 
    select(customer_id, sherlock_segment = proba_group) |> 
    mutate(sherlock_segment = str_replace_all(
      sherlock_segment, "percent", "%"
    ))

  sherlock_experimental_customers <- drive_ls("kasuki/2025-08-sherlock-experimental", type = "csv") |>
    pull(id) |> 
    map(function(file_id) { drive_read_string(file_id) |> read_csv() }) |> 
    bind_rows() |> 
    left_join(sherlock_run_august, by = "customer_id")
}

{
  sherlock_run_november <- read_sheet("https://docs.google.com/spreadsheets/d/1MTUB1X2V6naRvBa6EZB0ahHIfHqB4g73Tyc4haEu4E8/edit", sheet = "Borrower List") |> 
